...
import { analyzeContent, improveScript, generateScript, validateScript, validateYaml, validateJson, executeScript, addDocstrings, optimizePerformance, checkSecurity, generateApiTestsFromScript, refactorSelection, refactorYaml, optimizeCodeSnippet, generateBashTests } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const [bashContent, bashFns] = useUndoRedo<string>(() => localStorage.getItem('cortexstudio-bash') || INITIAL_SCRIPT);
  const [yamlContent, yamlFns] = useUndoRedo<string>(() => localStorage.getItem('cortexstudio-yaml') || INITIAL_YAML);
  const [jsonContent, jsonFns] = useUndoRedo<string>(() => localStorage.getItem('cortexstudio-json') || INITIAL_JSON);

  const editorStates = {
    bash: { content: bashContent, fns: bashFns, storageKey: 'cortexstudio-bash' },
    yaml: { content: yamlContent, fns: yamlFns, storageKey: 'cortexstudio-yaml' },
    json: { content: jsonContent, fns: jsonFns, storageKey: 'cortexstudio-json' },
  };
  ...
  useEffect(() => {
    try {
      const savedHistory = localStorage.getItem('cortexstudio-script-history');
      if (savedHistory) setScriptHistory(JSON.parse(savedHistory));

      const savedToken = localStorage.getItem('cortexstudio-github-token');
      if (savedToken) handleTokenSubmit(savedToken);
      
      const lastCheckKey = 'cortexstudio-last-kb-check';
  ...
  const handleSaveScript = useCallback(() => {
    ...
    if (activeEditor === 'bash') {
      setScriptHistory(prevHistory => {
          ...
          localStorage.setItem('cortexstudio-script-history', JSON.stringify(updatedHistory));
  ...
  const handleTokenSubmit = async (token: string) => {
    try {
      ...
      localStorage.setItem('cortexstudio-github-token', token);
      return true;
    } catch (error) {
      console.error("GitHub token validation failed:", error);
      setGithubUser(null);
      setGithubToken(null);
      localStorage.removeItem('cortexstudio-github-token');
      return false;
    }
  };
  ...
  const handleOptimizePerformance = async (codeToOptimize: string, range: { start: number; end: number }) => {
    const isSelection = range.start !== range.end;
    ...
    if (isSelection) {
      ...
      setResult('');
      setActiveView(ActiveView.Assistant);
      const title = t('optimizationTitle');
      setResultTitle(t('thinkingTitle', { title }));
  
      try {
        const { suggestedCode, explanation } = await optimizeCodeSnippet(codeToOptimize);
        setRefactorSuggestion({ originalSelection: range, suggestedCode });
        setResult(explanation);
        setResultTitle(title);
      } catch (error) {
      ...
      handleApiCall(optimizePerformance, codeToOptimize, 'optimizationTitle');
    }
  };
  ...
  const handleGenerateTests = () => {
    if (activeEditor !== 'bash') return;
    handleApiCall(generateBashTests, activeContent, 'generatedTestsTitle', true);
  };
        ...
        <main className="flex-grow p-4 lg:p-6 flex flex-col lg:flex-row gap-6 min-w-0">
          ...
          `}>
            <ScriptEditor
              ...
              onGenerateTests={handleGenerateTests} // New prop
            />
          </div>
          <div className={`
