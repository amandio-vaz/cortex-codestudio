...
const LOCAL_STORAGE_KEY = 'cortexstudio-api-collection';
    ...
    const renderBody = () => {
        ...
        if (contentType.includes('application/json')) {
            try {
                ...
                return <pre className="text-sm whitespace-pre-wrap"><code>{JSON.stringify(json, null, 2)}</code></pre>;
            } catch (e) {
                // If parsing fails despite the header, show it as text with an error message.
                const errorMessage = e instanceof Error ? e.message : t('apiInvalidJsonGeneric');
                return <pre className="text-sm whitespace-pre-wrap text-red-400"><code>{`${t('apiInvalidJson')} ${errorMessage}\n\n${response.body}`}</code></pre>;
            }
        } else if (contentType.includes('text/html')) {
            // Use a sandboxed iframe to render HTML safely.
            return (
                <iframe
                    srcDoc={response.body}
                    sandbox="" // most restrictive sandbox
                    className="w-full h-full border-0 bg-white rounded-md"
                    title="HTML Response Viewer"
                />
            );
        } else {
            // Fallback for plain text, xml, etc.
            return <pre className="text-sm whitespace-pre-wrap"><code>{response.body}</code></pre>;
        }
    };
