...
const Chatbot: React.FC = () => {
  ...
  const [systemInstruction, setSystemInstruction] = useState<string>(() => {
    return localStorage.getItem('cortexstudio-chat-instruction') || 'Você é um assistente prestativo e amigável, especializado em scripts Bash e ferramentas de linha de comando. Responda em Markdown bem formatado.';
  });

  const [messages, setMessages] = useState<ChatMessage[]>(() => {
      try {
          const savedMessages = localStorage.getItem('cortexstudio-chat-history');
  ...
  useEffect(() => {
    localStorage.setItem('cortexstudio-chat-instruction', systemInstruction);
  }, [systemInstruction]);

  useEffect(() => {
    localStorage.setItem('cortexstudio-chat-history', JSON.stringify(messages));
  }, [messages]);
