...
const GeneratorView: React.FC<GeneratorViewProps> = ({ onGenerate, isLoading }) => {
  ...
  const [prompt, setPrompt] = useState<string>(() => {
    return localStorage.getItem('cortexstudio-generator-prompt') || 'Crie um script que lista todos os arquivos no diretório atual e os ordena por tamanho.';
  });
  
  const [systemInstruction, setSystemInstruction] = useState<string>(() => {
    return localStorage.getItem('cortexstudio-generator-instruction') || '';
  });

  const [requiredCommands, setRequiredCommands] = useState<string>(() => {
    return localStorage.getItem('cortexstudio-generator-commands') || '';
  });

  // Debounced auto-save for generator fields
  useEffect(() => {
    const handler = setTimeout(() => {
      localStorage.setItem('cortexstudio-generator-prompt', prompt);
      localStorage.setItem('cortexstudio-generator-instruction', systemInstruction);
      localStorage.setItem('cortexstudio-generator-commands', requiredCommands);
    }, 1000);
