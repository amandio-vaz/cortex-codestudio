...
import SnippetSelectorModal from './SnippetSelectorModal';
...
interface ScriptEditorProps {
  ...
  onOptimizePerformance: (codeToOptimize: string, range: { start: number; end: number }) => void;
...
const ScriptEditor: React.FC<ScriptEditorProps> = ({ content, setContent, onSave, undo, redo, canUndo, canRedo, onAnalyze, onImprove, onValidate, onExecute, onAutoValidate, onToggleHistoryPanel, onToggleGithubPanel, isLoading, notificationMessage, issues, isFullscreen, onToggleFullscreen, isCollapsed, onToggleCollapse, onAddDocstrings, onOptimizePerformance, onCheckSecurity, onTestApi, onClearScript, onRunInTerminal, onRefactorSelection, onRefactorYaml, githubUser, currentGistId, onUpdateGist, onOpenExecutionConfig, activeEditor, onSetEditor, onGenerateTests }) => {
  ...
  const InsertSnippetIcon = getIconComponent('insertSnippet');
  const GenerateTestsIcon = getIconComponent('generateTests'); // New icon instance
  ...
  const commands = useMemo((): Command[] => [
    ...
    { id: 'optimize', name: t('buttonOptimizePerformance'), icon: <OptimizePerformanceIcon />, action: () => onOptimizePerformance(content.substring(selection.start, selection.end) || content, selection), disabled: isLoading || !content || activeEditor !== 'bash' },
    ...
    { id: 'insertSnippet', name: t('buttonInsertSnippet'), icon: <InsertSnippetIcon />, action: () => setIsSnippetModalOpen(true), disabled: isLoading || activeEditor !== 'bash' },
    { id: 'generateTests', name: t('buttonGenerateTests'), icon: <GenerateTestsIcon />, action: onGenerateTests, disabled: isLoading || !content || activeEditor !== 'bash' }, // New command
  ], [t, content, isLoading, onAnalyze, onImprove, onValidate, onRefactorYaml, onExecute, onAddDocstrings, onOptimizePerformance, onCheckSecurity, onTestApi, activeEditor, selection, onGenerateTests]);
          ...
          <div className="mt-4 flex flex-col flex-grow min-h-0">
              ...
              <div className="mt-4 flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                      ...
                      <Tooltip text={t('tooltipInsertSnippet')}><button onClick={() => setIsSnippetModalOpen(true)} disabled={isLoading || activeEditor !== 'bash'} className="w-full flex items-center justify-center px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed bg-gray-100 hover:bg-gray-200 text-gray-800 dark:text-white dark:bg-gradient-to-br dark:from-gray-500 dark:to-gray-700 dark:hover:from-gray-600 dark:hover:to-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-200 dark:focus:ring-gray-800"><InsertSnippetIcon className="h-5 w-5 mr-2" /> {t('buttonInsertSnippet')}</button></Tooltip>
                      <Tooltip text={t('tooltipGenerateTests')}><button onClick={onGenerateTests} disabled={isLoading || !content || activeEditor !== 'bash'} className="w-full flex items-center justify-center px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed bg-purple-100 hover:bg-purple-200 text-purple-800 dark:text-white dark:bg-gradient-to-br dark:from-purple-500 dark:to-indigo-500 dark:hover:from-purple-600 dark:hover:to-indigo-600 focus:ring-4 focus:outline-none focus:ring-purple-200 dark:focus:ring-purple-800"><GenerateTestsIcon className="h-5 w-5 mr-2" /> {t('buttonGenerateTests')}</button></Tooltip> {/* New button */}
                  </div>
              </div>
          </div>
        </>
