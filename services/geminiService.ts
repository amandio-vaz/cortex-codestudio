...
export const optimizeCodeSnippet = async (snippet: string): Promise<{ suggestedCode: string, explanation: string }> => {
    try {
        const ai = getAi();
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-pro',
            contents: `You are a systems performance engineer specializing in Bash. Analyze the following Bash code snippet for performance bottlenecks and optimization opportunities.

Your task is to rewrite the snippet to be more efficient, for example, by reducing process calls, using shell built-ins, or improving algorithms.

Respond ONLY with a JSON object that adheres to the provided schema. The 'explanation' should be a detailed, Markdown-formatted list explaining EACH optimization and why it improves performance.
...
`,
            config: {
                ...
                responseSchema: {
                    ...
                    properties: {
                        suggestedCode: {
                            type: Type.STRING,
                            description: "The optimized Bash code snippet."
                        },
                        explanation: {
                            type: Type.STRING,
                            description: "A detailed, Markdown-formatted explanation of the performance optimizations made."
                        }
                    },
                    required: ['suggestedCode', 'explanation']
                }
        ...
        if (!result.suggestedCode && result.explanation) {
             return { suggestedCode: snippet, explanation: result.explanation };
        }

        return result;
    } catch (error) {
        console.error("Error optimizing code snippet:", error);
        throw error;
    }
};
...
export const generateBashTests = async (script: string): Promise<string> => {
    try {
        const ai = getAi();
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-pro',
            contents: `Você é um engenheiro sênior especializado em testes de Bash. Sua tarefa é gerar uma suíte de testes de unidade para o script Bash fornecido, utilizando o framework 'bats-core'.

**Requisitos:**
1.  **Formato Bats-core:** Os testes DEVE seguir a sintaxe e as melhores práticas do 'bats-core'.
2.  **Cobertura Abrangente:** Gere testes que cubram:
    -   **Funcionalidade Básica:** Verifique se as funções e a lógica principal do script funcionam conforme o esperado com entradas válidas.
    -   **Casos de Borda:** Inclua testes para cenários extremos ou limites (ex: entradas vazias, números grandes/pequenos, arquivos não existentes).
    -   **Condições de Erro:** Teste como o script lida com erros (ex: argumentos inválidos, comandos ausentes, permissões negadas). Verifique se ele retorna os códigos de saída apropriados e mensagens de erro claras.
3.  **Setup/Teardown:** Utilize as funções \`setup()\` e \`teardown()\` do bats-core para preparar o ambiente de teste e limpá-lo, se necessário (ex: criar/remover arquivos temporários).
4.  **Asserções:** Use asserções como \`run\`, \`[ "$status" -eq 0 ]\`, \`[ "$output" = "esperado" ]\`, \`[ "$lines" -eq N ]\`, etc.
5.  **Comentários:** Adicione comentários explicativos em Português do Brasil (pt-BR) para cada bloco de teste e asserção.
6.  **Script Original:** O script a ser testado será salvo como um executável temporário e adicionado ao PATH do teste, de modo que os testes possam chamá-lo diretamente pelo nome (ex: \`run meu_script arg1 arg2\`).

**Formato de Saída:** Retorne APENAS o conteúdo completo do arquivo .bats dentro de um único bloco de código \`\`\`bash. Não inclua nenhuma explicação fora do bloco de código.

**Script para Gerar Testes:**
\`\`\`bash
${script}
\`\`\`
`
        ...
        console.error("Error generating Bash tests:", error);
        throw error;
    }
};