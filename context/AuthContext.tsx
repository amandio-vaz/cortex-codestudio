...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    try {
      const storedUser = localStorage.getItem('cortexstudio-user');
      const storedToken = localStorage.getItem('cortexstudio-token');
  ...
  const login = useCallback(async (email: string, otp: string): Promise<boolean> => {
    ...
    setUser(mockUser);
    setToken(mockToken);
    localStorage.setItem('cortexstudio-user', JSON.stringify(mockUser));
    localStorage.setItem('cortexstudio-token', mockToken);
    setIsLoading(false);
    return true;
  }, []);
  ...
  const logout = useCallback(() => {
    setUser(null);
    setToken(null);
    localStorage.removeItem('cortexstudio-user');
    localStorage.removeItem('cortexstudio-token');
  }, []);
