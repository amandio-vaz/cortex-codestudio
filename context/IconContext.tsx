...
import { ICON_LIBRARY, DEFAULT_ICONS, FlaskIcon } from '../icons';
...
export const IconProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [iconSettings, setIconSettings] = useState<IconSettings>(() => {
    try {
      const savedSettings = localStorage.getItem('cortexstudio-icons');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('cortexstudio-icons', JSON.stringify(iconSettings));
    } catch (error) {
